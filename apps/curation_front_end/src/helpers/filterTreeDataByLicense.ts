...
export const filterTreeDataByLicense = (root: TreeNode[], licenseSearchText: string): TreeNode[] => {
    ...
    const recursiveFilter = (node: TreeNode): TreeNode | null => {
        ...
        const hasLicense = node.file?.licenseFindings?.some(licenseFinding =>
            licenseFinding.licenseExpressionSPDX?.toLowerCase().includes(licenseSearchText.toLowerCase())
        );
