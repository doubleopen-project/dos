...
export const updateHasLicenseFindings = (tree: TreeNode[], licenseSearchText: string) => {
    ...
    const updateNode = (node: TreeNode) => {
        ...
        if (node.file?.licenseFindings) {
            for (const licenseFinding of node.file.licenseFindings) {
                if (licenseFinding?.licenseExpressionSPDX?.toLowerCase().includes(licenseSearchText.toLowerCase())) {
