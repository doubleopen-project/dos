...
export const sortTree = (array: TreeNode[]) => {
  const dotFolders = array.filter(
    (item) =>
      item.children && item.children.length > 0 && item.name.startsWith("."),
  );
  const capitalFolders = array.filter(
    (item) =>
      item.children &&
  ...
  const dotFiles = array.filter(
    (item) =>
      (!item.children || item.children.length === 0) &&
      item.name.startsWith("."),
  );
  const capitalFiles = array.filter(
    (item) =>
      (!item.children || item.children.length === 0) &&
      !item.name.startsWith(".") &&
      item.name[0] === item.name[0].toUpperCase(),
