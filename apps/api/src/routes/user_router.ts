...
userRouter.delete("/license-conclusion/:id", async (req, res) => {
  try {
    ...
    if (!licenseConclusionUserId)
      res
        .status(404)
        .json({ message: "License conclusion to delete not found" });
    // Make sure that the license conclusion belongs to the user or the user is admin
    if (req.user.role === "ADMIN" || req.user.id === licenseConclusionUserId) {
    ...
    if (error.code === "P2025") {
      return res.status(404).json({
        message: "License conclusion with the requested id does not exist",
      });
    } else {
      res.status(500).json({ message: "Internal server error" });
    }
...
userRouter.post("/file", async (req, res) => {
  try {
    ...
    if (fileData) {
      ...
      res.status(400).json({
        message: "Bad request: File with the requested sha256 does not exist",
      });
    }
  } catch (error) {
    console.log("Error: ", error);
          ...
          return {
            ...
            file: {
              licenseFindings: filetree.file.licenseFindings.map(
                (licenseFinding) => {
                  return {
                    licenseExpressionSPDX: licenseFinding.licenseExpressionSPDX,
                  };
                },
              ),
            },
