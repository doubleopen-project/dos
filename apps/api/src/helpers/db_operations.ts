...
export const getPackageResults = async (
  ...
  if (queriedPackage) {
    if (queriedPackage.scanStatus === "pending") {
      const scannerJob = await dbQueries.findMostRecentScannerJobByPackageId(
        queriedPackage.id,
      );
...
export const deletePackageDataByPurl = async (
  ...
  if (packageIds && packageIds.length > 0) {
    ...
    if (fileHashes && fileHashes.length > 0) {
      ...
      console.log(
        "deletedCopyrightFindings count: " + deletedCopyrightFindings.count,
      );
  ...
  try {
        ...
        const queryTask = (async () => {
          if (file.type === "file" && file.sha256) {
            ...
            if (!dbFile) {
              ...
              // Delete old findings
              await dbQueries.deleteLicenseFindingsByFileHashes([file.sha256]);
              await dbQueries.deleteCopyrightFindingsByFileHashes([
                file.sha256,
              ]);
              await dbQueries.deleteScanIssuesByFileHashes([file.sha256]);
            }
    ...
    if (newJobFilesList.length > 0) {
      try {
        ...
        if (
          newTimeout <= (parseInt(process.env.TIMEOUT_MAX as string) || 3600)
        ) {
          ...
          if (addedToQueue) {
            console.log(
              newScannerJob.id + ': Updating ScannerJob state to "queued"',
            );
